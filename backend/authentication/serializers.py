...
class UserSerializer(serializers.ModelSerializer):
    ...
    company = serializers.SerializerMethodField('company_stuff')

    def company_stuff(self, user):
        company_stuff = CompanyStuff.objects.filter(stuff=user).first()
        if company_stuff:
            return company_stuff.company.name
        else:
            return None

    company_id = serializers.SerializerMethodField('company_id_stuff')

    def company_id_stuff(self, user):
        company_stuff = CompanyStuff.objects.filter(stuff=user).first()
        if company_stuff:
            return company_stuff.company.id
        else:
            return None
    ...
    class Meta:
        ...
        fields = tuple(User.REQUIRED_FIELDS) + (
            User._meta.pk.name,
            User.USERNAME_FIELD, 'country', 'city', 'first_name', 'last_name', 'phone', 'address', 'company', 'company_id'
        )
...
class UserCreateSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        model = User
        fields = tuple(User.REQUIRED_FIELDS) + (
            User.USERNAME_FIELD, User._meta.pk.name, 'password', 'country', 'city', 'first_name', 'last_name', 'phone', 'address'
        )
